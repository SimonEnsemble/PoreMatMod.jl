...
# ╔═╡ 2a45fa16-4bff-11ec-1f85-43656d3db3c6
using PoreMatMod

# ╔═╡ 80eb11ea-d02a-4433-8822-0aa26473d79b
begin
	rc[:paths][:crystals] = joinpath(pwd(), "pac")
	rc[:paths][:moieties] = joinpath(pwd(), "pac")
end

# ╔═╡ 6ed2e1d9-cc1e-479c-b790-63411568461d
begin
...
# ╔═╡ e73dac68-7fdd-4422-888f-26e6ffdfb13e
query = moiety("naphthyl_fragment.xyz")

# ╔═╡ 0eaa7b0c-294f-46f9-b1c1-bf5bff9936d7
search = query in parent

# ╔═╡ e871f757-b1df-45ab-8244-77f9f7f3260c
replacement = moiety("F_naphthyl_fragment.xyz")

# ╔═╡ 24344cb6-5d16-464a-b36c-04f621f23d22
child = replace(parent, query => replacement, nb_loc=1)

# ╔═╡ 7d658bc3-3a57-4b10-a3b1-e4121c855bcd
child2 = replace(parent, query => replacement, nb_loc=2)

# ╔═╡ e6828f16-e13c-4d4f-af3b-47811f443c09
begin
	translate_by!(child.atoms.coords, Frac([0.7, 0.3, 0.6]))
	wrap!(child)
	write_cif(child, "pac/child1.cif")
end

# ╔═╡ 7a7d130b-169c-434a-aa7c-180ad8fa9c55
begin
	translate_by!(child2.atoms.coords, Frac([0.7, 0.3, 0.6]))
	wrap!(child2)
	write_cif(child2, "pac/child2.cif")
end

# ╔═╡ c521aa87-116d-4889-a113-02172da2b374
write_cif(Crystal("child2.cif", remove_duplicates=true), "pac/child2.5.cif")
